...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'data/tools.json'; REPORT=ROOT/'link-report.json'
...
def check(item):
    ...
    for method in ('HEAD','GET'):
        try:
            ...
            with urlopen(req,timeout=12) as r: result['status']=r.status; result['ok']=reachable(r.status); break
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as pool: results=list(pool.map(check,tools))
    dead=[r for r in results if not r['ok'] and not r.get('needs_review')]; review=[r for r in results if r.get('needs_review')]
    report={'checked_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'total':len(results),'ok':len(results)-len(dead)-len(review),'needs_review':len(review),'failed':len(dead),'failure_rate':round(len(dead)/len(results),4) if results else 1,'results':results}
    REPORT.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(f"Checked {len(results)} URLs: {report['ok']} reachable, {report['needs_review']} review, {report['failed']} dead")
    if results and len(dead)/len(results)>.25: raise SystemExit('More than 25% of provider URLs are confirmed dead; refusing deployment')
